...
export function PreServiceQuestionnaire() {
  ...
  return (
    <SectionShell id="diagnostico" tone="solid" className={bypassQuestionnaire ? "!py-8 sm:!py-12" : undefined}>
      <div
        className={
          bypassQuestionnaire
            ? "relative mx-auto grid max-w-2xl px-4 sm:px-6 lg:px-8"
            : "relative mx-auto grid max-w-7xl gap-8 px-4 sm:gap-10 sm:px-6 lg:grid-cols-[0.82fr_1.18fr] lg:px-8"
        }
      >
        {bypassQuestionnaire ? null : (
          <div>
            <SectionIntro
        ...
        <PreServiceQuestionnaireForm
          bypassQuestionnaire={bypassQuestionnaire}
          onBypassChange={setBypassQuestionnaire}
        />
      </div>
    </SectionShell>
  );
